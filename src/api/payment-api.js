...
const setApiURLUser = backendConfig.base + "/user";
...
export async function getPlanLists() {
  const response = await axios.get(`${setApiURL}/all-category-packages`);
...
export async function getWithdrawalHistory() {
  const response = await axios.get(`${setApiURLUser}/withdrawals-history`, {
    headers: {
      Authorization: `Bearer ${token}`,
    },
    withCredentials: true,
...
export async function approveUserPayment(id) {
  const response = await axios.post(
    `${adminApi}/transaction/details-match/${id}`,
    { status: "Completed" }
  );
  return response;
}
...
export async function getCompletePlanByWallet() {
  const response = await axios.get(`${adminApi}/payment-wallets-transaction`);
  return response?.data;
}
...
export async function sendWithdrawalresponse(payload) {
  console.log(token);
  const response = await axios.post(
    `${setApiURLUser}/withdraw-request`,
    payload,
    {
      headers: {
        Authorization: `Bearer ${token}`,

...
import { sendWithdrawalOTP } from "../../api/user-api"; 
const Withdrawal = () => {
  ...
  const handleWithdrawClick = async () => {
    ...
    if (amount > userInfo?.user?.correntIncome) {
      return SwalError.fire({
        title: "Error",
    ...
    try {
      setLoading(true); 
      const otpResponse = await sendWithdrawalOTP();
      ...
      if (!window.ethereum) throw new Error("MetaMask or SafePal wallet not found.");
      ...
      // console.log("Withdrawal API Response:", resData);

      if (resData?.success) {
        Swal.fire({
          icon: "success",
          title: "Withdrawal Request Successful",
          text: `Transaction initiated for ${amount} USDT. It will be settled in 24 hours.`,
          confirmButtonText: "Ok",
        ...
        <div className="ss-card half martop">
          <div className="top">
            <h5 className="heading">
              Main Wallet: $
              {userInfo?.correntIncome?.toFixed(2) || 0}
            </h5>
          </div>
